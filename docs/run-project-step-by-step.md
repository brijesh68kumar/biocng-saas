...
3. Dashboard plus module pages (feedstock + farmers + collection centers + vehicles + land parcels + crop plans + harvest batches + center receipt lots + center stock ledger live)
4. Logout button to end session
5. In `Feedstock Types`, `Farmers`, `Collection Centers`, `Vehicles`, `Land Parcels`, `Crop Plans`, `Harvest Batches`, `Center Receipt Lots`, and `Center Stock Ledger` pages you can create/list operational records from backend API
